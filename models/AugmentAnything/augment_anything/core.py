...
class AugmentAnything:
    ...
    def _blend_segment(self, target_img, source_img, target_mask, source_mask):
        ...
        # Resize source image and mask to target size
        if source_img.shape[:2] != (H_tgt, W_tgt):
            source_img = cv2.resize(source_img, (W_tgt, H_tgt))
            source_mask = cv2.resize(
                source_mask.astype(np.uint8), (W_tgt, H_tgt),
                interpolation=cv2.INTER_NEAREST
            ).astype(bool)

        if target_mask.shape != (H_tgt, W_tgt):
            target_mask = cv2.resize(
                target_mask.astype(np.uint8), (W_tgt, H_tgt),
                interpolation=cv2.INTER_NEAREST
            ).astype(bool)

        # Combine source and target masks
        # Only blend where BOTH indicate valid regions
        combined_mask = (target_mask.astype(np.float32) * source_mask.astype(np.float32))

        # Create smooth blend mask for seamless compositing
        ...
        # Blend: paste source segment pixels at target location
